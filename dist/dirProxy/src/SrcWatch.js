...
var SrcWatch = /** @class */ (function () {
    ...
    SrcWatch.start = function () {
        chokidar_1.default.watch(MainConfig_1.default.config.src).on('all', function (_e, _url) {
            //更新缓存文件模块
            SrcCache_1.default.updateModule(_url);
            //取相对路径
            _url = _url.replace(path_1.join(MainConfig_1.default.config.src, '/'), '/');
            //发送webSocket消息
            WebSocket_1.default.send("src\u4EE3\u7801" + _e + "@" + path_1.join(_url) + "\u2714\uFE0F", EWebSocketMesType_1.EWebSocketMesType.contentUpdate);
        });
    };
    return SrcWatch;
}());
