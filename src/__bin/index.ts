...
(async () => {
    ...
    switch (true) {
        case Boolean(options.version):
            console.log(
                chalk.green('当前layabox-esbuild的版本@ ') +
        ...
        case Boolean(options.help):
            ...
            console.log(
                ...
                chalk.gray('查看当前layabox-esbuild工具版本'),
            );
            console.log(chalk.blue('-h --help'), chalk.gray('查看帮助信息'));
            console.log(
                chalk.blue('-i --init'),
                chalk.gray('在当前执行目录初始化配置文件'),
            );
        ...
        case Boolean(options.init):
            ...
            fs.stat(_configUrl, (err, stats) => {
                if (!err && stats.isFile()) {
                    console.log(
                        chalk.red('配置文件已经存在，请在该文件上添加配置。', _configUrl),
                    );
                    return;
                }
                //
